...
function handleImageToAndroid(config) {
  ...
  var resolutionStr = ['drawable-mdpi', 'drawable-hdpi', 'drawable-xhdpi', 'drawable-xxhdpi'];
  var params = {
...
function handleImageToiPhone(config) {
  ...
  if (config.isAppIcon) {
    result = result && toAppIcon();
    result = result && toAppStoreIcon();
    result = result && toiPadIcon();
  } else {
    result = result && toiPhoneLaunch();
  }

  function toAppIcon() {
    params.fileName = config.fileName + '_2x';
    params.width = params.height = 120;
    return generateImage(params);
  }

  function toAppStoreIcon() {
    params.fileName = config.fileName + '_app_store';
    params.width = params.height = 1024;
    return generateImage(params);
  }

  function toiPadIcon() {
    params.fileName = config.fileName + '_ipad';
    params.width = params.height = 152;
    return generateImage(params);
  }
